...
function sendLike(id) {
    const current_label = $(`#likeText`+id).text()
    const sendLike = $.post("includes/handlers/send_like.php", 
        {userLoggedIn:userLoggedIn, id:id}, 
        function(response){
            updateLikes(id);
            console.log(current_label);
            $(`#likeText`+id).text(current_label == 'Like' ? 'Unlike' : 'Like');
            if(current_label=='Like'){
                $(`#likeBtn`+id).addClass('likeBtn');
            }else{
                $(`#likeBtn`+id).removeClass('likeBtn');
            }
    });
}
    ...
    const sendComment = $.post("includes/handlers/send_comment.php", 
        ...
        if(response !== "No text") {
            ...
            const loadComment = $.post("includes/handlers/load_comment.php", 
                ...
                $("#loadComment"+id).append(newComment);
