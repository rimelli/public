			...
			while($row = $data_query->fetch()) {
				...
				if($user_logged_obj->isFollowing($added_by) || $user_to = $added_by){
					...
					if($userLoggedIn == $added_by)
						$delete_button = "<button id='post$id'><a href='#' class='button gray ripple-effect ico' title='Delete Post' data-tippy-placement='top'><i class='icon-feather-trash-2'></i></a></button>";
					else
						$delete_button = "";
					...
					if($num_rows > 0) {
					    $like_button .= "<button id='like_button_$id' name='like_button' onclick='sendLike($id)' value='Like' style='background-color:blue;'>
											<a href='#' style='color:white;'><i class='icon-line-awesome-thumbs-up' style='color:white;'></i> Liked</a>
										</button>";
					}
					else {
					    $like_button .= "<button>
					...
					$str .= "<li><div class='freelancer-overview manage-candidates'>
