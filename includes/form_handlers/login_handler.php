...
$errors = false; //Holds error messages
if (isset($_POST['login_button'])) {
    ...
    if ($check_database_query->rowCount() == 1) {
        ...
        if (password_verify($password, $check_login_query['password'])) {
            ...
            echo json_encode(array('status' => 'success', 'message' => 'Welcome!', 'url' => 'index.php'));
            exit();
        }
        ...
        if ($errors) {
            die(json_encode(array('status' => 'error', 'message' => 'Email or password was incorrect')));
        }
    }
}
?>